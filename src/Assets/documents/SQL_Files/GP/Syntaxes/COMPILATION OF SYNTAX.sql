...
(SELECT country, customer_id, SUM(invoice_amount_usd) as total_amount_usd, disputed, dispute_lost FROM gp1
...
SELECT gp1_4.country, gp1_4.customer_id, gp1_4.total_amount_usd AS revenue_gain_from_disputes,
gp1_3.total_amount_usd AS revenue_lost, gp1_4.total_amount_usd + gp1_3.total_amount_usd AS expected_total_revenue,
gp1_1.outliers AS z_scores FROM gp1_4
JOIN gp1_3
ON gp1_4.customer_id = gp1_3.customer_id
JOIN gp1_1
ON gp1_4.customer_id = gp1_1.customer_id
WHERE gp1_1.outliers > 2.576 OR gp1_1.outliers < -2.576
ORDER BY gp1_4.total_amount_usd DESC

----------------------------------------
CREATE TABLE gp1_9 AS
(SELECT country, customer_id, invoice_amount_usd, days_to_settle, days_late, disputed, dispute_lost FROM gp1)
